...
# B: 1/4 Kelly ≈ 1%
STAKE_PCT = {"S+": 0.04, "S": 0.03, "A+": 0.025, "A": 0.02, "B+": 0.02, "B": 0.01, "C": 0.005}
...
def main():
    ...
    # Accept tiers S+, S, A+, A, B+, B
    df = df[df["tier"].isin(["S+", "S", "A+", "A", "B+", "B"])].copy()
    print(f"Tier S+/S/A+/A/B+/B: {len(df)}")
    if df.empty:
        print("No candidates.")
        return
    ...
    tier_prio = {"S+": 6, "S": 5, "A+": 4, "B+": 3.5, "A": 3, "B": 2, "C": 1}
